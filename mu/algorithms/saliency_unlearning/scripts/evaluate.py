...
def main():
    ...
    parser = ArgumentParser(description="SaliencyUnlearning Evaluation and Sampling")
    ...
    #logger setuup
    log_file = os.path.join(logs_dir, f"SaliencyUnlearning_evalaute_{config.get('theme')}.log")
    logger = setup_logger(log_file=log_file, level=logging.INFO)
    logger.info("Starting Saliency Unlearning Evalution Framework")

    evaluator = SaliencyUnlearningEvaluator(config)
    evaluator.run()
