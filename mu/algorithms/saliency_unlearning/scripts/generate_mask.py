...
def main():
    parser = argparse.ArgumentParser(
        prog="GenerateMask",
        description="Generate saliency mask using MaskingAlgorithm.",
    )

    parser.add_argument(
        "--c_guidance",
        help="Guidance scale used in loss computation",
        type=float,
        default=7.5,
    )
    parser.add_argument(
        "--batch_size", help="Batch size used for mask generation", type=int, default=4
    )
    parser.add_argument(
        "--ckpt_path", help="Checkpoint path for the model", type=str, required=True
    )
    parser.add_argument(
        "--config_path",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        help="Output directory for the generated mask",
        type=str,
        required=False,
        default="data/mask",
    )
    parser.add_argument(
        "--threshold", help="Threshold for mask generation", type=float, default=0.5
    )
    # Dataset directories
    parser.add_argument(
        "--original_data_dir",
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="/home/ubuntu/Projects/msu_unlearningalgorithm/mu/algorithms/saliency_unlearning/data",
        help="Directory where the new datasets will be saved.",
    )

    parser.add_argument(
        "--image_size", help="Image size for training", type=int, default=512
    )
    parser.add_argument(
        "--lr", help="Learning rate for optimizer", type=float, default=1e-5
    )
    parser.add_argument(
    ...
    if os.path.exists(mask_path):
        print(
            f"Mask for threshold {args.threshold} already exists at {mask_path}. Skipping."
        )
        return
