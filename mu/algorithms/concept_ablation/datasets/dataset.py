...
class ConceptAblationDataset(Dataset):
    ...
    def _load_final_data(self) -> Tuple[List[str], List[str]]:
        ...
        if not (images_path.exists() and prompts_path.exists()):
            raise FileNotFoundError("Images or prompts file not found after generation and filtering step.")
        ...
        if len(image_paths) != len(prompts):
            raise ValueError("Mismatch between number of images and prompts in the final dataset.")
