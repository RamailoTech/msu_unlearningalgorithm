    ...
    def __init__(
        ...
        if reg_datapath is not None:
            if os.path.isdir(reg_datapath):
                self.image_paths2 = [
                    os.path.join(reg_datapath, file_path)
                    for file_path in os.listdir(reg_datapath)
                    if isimage(file_path)
                ]
            else:
    ...
    def __getitem__(self, i):
        ...
        if hasattr(self, "caption_target_list"):
            if self.style or self.caption_target_list[0].startswith("*"):
                self.caption_target = self.caption_target_list[
                    i % len(self.caption_target_list)
                ]
            else:
                self.caption_target = self.caption_target_list[
                    (i % min(self._length1, len(self.caption))) // 1000
                ]
        ...
        if i >= self._length2 or self._length2 == 0:
            ...
            if isinstance(self.caption, str):
                example["caption"] = np.random.choice(self.templates_small).format(
                    self.caption
                )
            else:
                example["caption"] = self.caption[
                    i % min(self._length1, len(self.caption))
                ]
            if self.caption_target is not None:
                if self.style:
                    example["caption_target"] = example["caption"]
                    example["caption"] = get_styled_prompt(
                        self.caption_target, example["caption_target"]
                    )
                else:
                    example["caption_target"] = example["caption"]
                    ...
                    if general == "*":
                        ...
                        example["caption"] = (
                            example["caption_target"]
                            .strip()
                            .lower()
                            .replace(general, target)
                        )
        else:
        ...
        if i > self._length2 or self._length2 == 0:
            if self.aug:
                if np.random.randint(0, 3) < 2:
                    ...
                    random_scale = np.random.randint(
                        int(1.2 * self.size), int(1.4 * self.size)
                    )
            ...
            if random_scale < 0.6 * self.size:
                ...
                cx = np.random.randint(
                    random_scale // 2, self.size - random_scale // 2 + 1
                )
                ...
                image = image.resize(
                    (random_scale, random_scale), resample=self.interpolation
                )
                ...
                mask[
                    (cx - random_scale // 2) // 8
                    + 1 : (cx + random_scale // 2) // 8
                    - 1,
                    (cy - random_scale // 2) // 8
                    + 1 : (cy + random_scale // 2) // 8
                    - 1,
                ] = 1.0
                ...
                example["caption"] = add_to_caption + example["caption"]
                cx = np.random.randint(
                    self.size // 2, random_scale - self.size // 2 + 1
                )
                cy = np.random.randint(
                    self.size // 2, random_scale - self.size // 2 + 1
                )
