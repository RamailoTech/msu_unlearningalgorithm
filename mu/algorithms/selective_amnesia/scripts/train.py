...
def main():
    ...
    parser.add_argument(
        "--ckpt_path", type=str, required=True, help="Path to SD v1.4 ckpt"
    )
    parser.add_argument(
        "--fim_path",
        type=str,
        required=True,
        help="Path to precomputed FIM (full_fisher_dict.pkl)",
    )
    parser.add_argument(
        "--surrogate_data_dir",
        type=str,
        required=True,
        help="Path to q(x|c_f) surrogate dataset",
    )
    parser.add_argument(
        "--output_dir", type=str, default="results", help="Directory to save results"
    )
    parser.add_argument(
        "--devices", type=str, default="0", help="CUDA devices (comma-separated)"
    )
    ...
    parser.add_argument(
        "--output_name", type=str, default="selective_amnesia_model.pth"
    )
