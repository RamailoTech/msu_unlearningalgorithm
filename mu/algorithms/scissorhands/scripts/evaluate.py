...
def main():
    ...
    parser = ArgumentParser(description="Scisssor Hands Evaluation and Sampling")
    ...
    #logger setuup
    log_file = os.path.join(logs_dir, f"scissor_hands_evalaute_{config.get('theme')}.log")
    logger = setup_logger(log_file=log_file, level=logging.INFO)
    logger.info("Starting Scissor hands Evalution Framework")

    evaluator = ScissorHandsEvaluator(config)
    evaluator.run()
