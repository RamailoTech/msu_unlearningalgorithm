...
def main():
    ...
    parser.add_argument(
        "--config_path",
        type=str,
        default="config/train_ti.yaml",
        help="Path to the configuration YAML file.",
    )
    parser.add_argument(
        "--pretrained_path", type=str, help="Path to the pretrained diffuser directory."
    )
    parser.add_argument("--theme", type=str, help="Theme or concept to unlearn.")
    parser.add_argument(
        "--output_dir", type=str, help="Directory to save outputs and checkpoints."
    )
    ...
    parser.add_argument(
        "--instance_data_dir", type=str, help="Directory containing instance images."
    )
    parser.add_argument(
    ...
    config.update(
        {
            ...
            "output_dir": os.path.join(
                args.output_dir or config.get("output_dir", ""),
                args.theme or config.get("theme"),
            ),
            "steps": args.steps or config.get("steps", 500),
            "lr": args.lr or config.get("lr", 1e-4),
            "instance_data_dir": os.path.join(
                args.instance_data_dir or config.get("instance_data_dir", "data"),
                args.theme or config.get("theme"),
            ),
