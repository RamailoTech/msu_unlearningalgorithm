...
def main():
    ...
    parser = ArgumentParser(description="Erase diff Evaluation and Sampling")
    ...
    #logger setup
    log_file = os.path.join(logs_dir, f"erase_diff_evalaute_{config.get('theme')}.log")
    logger = setup_logger(log_file=log_file, level=logging.INFO)
    logger.info("Starting Erase Diff Evalution Framework")
