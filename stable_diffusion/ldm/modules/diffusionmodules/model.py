...
class Encoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        attn_type="vanilla",
        **ignore_kwargs,
    ):  # the usage of ignorekwargs makes Encoder and Decoder shares the same constructor parameter
        super().__init__()
        if use_linear_attn:
            attn_type = "linear"
        self.ch = ch  # 128
        ...
        for i_level in range(self.num_resolutions):  # 4
            ...
            for i_block in range(self.num_res_blocks):  # 2
                ...
                block_in = block_out
                if (
                    curr_res in attn_resolutions
                ):  # [], not used, no attention between ResBlocks
                    attn.append(make_attn(block_in, attn_type=attn_type))
            down = nn.Module()
            down.block = block
            down.attn = attn
...
class Decoder(nn.Module):
    def __init__(
        ...
        **ignorekwargs,
        ...
        self.temb_ch = 0
        self.num_resolutions = len(
            ch_mult
        )  # [1, 2, 4, 4], ch_mult is shared by Encoder
        self.num_res_blocks = (
            num_res_blocks  # 2, the number of ResBlocks in one upsample block
        )
        self.resolution = resolution
        ...
        for i_level in reversed(range(self.num_resolutions)):  # 4
            ...
            for i_block in range(self.num_res_blocks + 1):  # 3
                ...
                if (
                    curr_res in attn_resolutions
                ):  # [], not used, no attention block between ResBlocks
                    attn.append(make_attn(block_in, attn_type=attn_type))
            up = nn.Module()
            up.block = block
            up.attn = attn
            ...
            self.up.insert(
                0, up
            )  # prepend to get consistent order, here the upsampling blocks as stored in a reversed order, and later in the forward function it is also used in a reversed order

        # end
        self.norm_out = Normalize(
            block_in
        )  # GroupNorm with channel (group) size of 32 by default
        self.conv_out = torch.nn.Conv2d(
            block_in, out_ch, kernel_size=3, stride=1, padding=1
        )
