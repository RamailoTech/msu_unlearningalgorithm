...
class AdvUnlearnModel(BaseModel):
    ...
    def load_model(self):
        ...
        if self.backend == "compvis":
            ...
            (self.model_orig, self.sampler_orig, 
                self.model, self.sampler, 
                self.vae, self.safety_checker, 
                self.feature_extractor) = get_models_for_diffusers(
                    self.diffusers_model_name_or_path, self.devices, self.target_ckpt, self.cache_path
                )
