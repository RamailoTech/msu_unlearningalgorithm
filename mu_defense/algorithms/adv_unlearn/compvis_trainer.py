...
# Note: We no longer import SoftPromptAttack directly.
        ...
        for i in pbar:
            if i % self.adv_prompt_update_step == 0:
                ...
                if i >= self.warmup_iter:
                    ...
                    # Build the attack configuration. (Adjust fields if necessary.)
                    attack_config = AdvAttackConfig(
                        ...
                        devices=",".join([d.strip() for d in self.config.get("devices", "cuda:0").split(',')]),
                        ...
                        attack_init_embd=None,  # Set as needed
                        attack_method=self.attack_method,
                        ddim_steps=self.ddim_steps,
                        ddim_eta=ddim_eta,
                        image_size=self.image_size,
                        adv_prompt_num=self.adv_prompt_num,
                        ...
                        target_ckpt="",
                        project=self.config.get("project_name", "default_project"),
                        experiment_name=self.config.get("experiment_name", "default_experiment")
                    )
                    adv_attack = AdvAttack(attack_config)
                    adv_word_embd, adv_input_ids = adv_attack.attack()
